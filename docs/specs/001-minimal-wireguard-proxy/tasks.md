...
- [x] 1. Initialize the Go CLI workspace and command skeleton for `cfwarp-cli`.
...
- [x] 2. Implement config/state path management and persistent data models.
...
- [x] 3. Implement the Cloudflare consumer registration client owned by this project.
...
- [x] 4. Implement settings loading from flags, env vars, and persisted defaults.
...
- [x] 5. Implement the backend abstraction and the first `singbox-wireguard` backend.
...
- [x] 6. Implement process supervision for `up`, `down`, and runtime cleanup.
...
- [x] 7. Implement `status` and lightweight health probing.
...
- [x] 8. Implement `endpoint test` for manual candidate validation.
...
- [x] 9. Add Docker packaging for the MVP deployment.
  - Create a multi-stage Dockerfile that builds `cfwarp-cli`, installs `sing-box` 1.13.5, and runs as a non-root user.
  - Implement an entrypoint that auto-registers on first start and reuses persisted state afterward.
  - Add docker-compose.yml and docker-compose.sidecar.yml deployment examples.
  - Requirements: 6.1, 6.2, 6.4, 7.4.

- [x] 10. Add example compose-backed integration tests and proxy-consumer tests.
  - Create automated integration coverage for the documented single-container deployment (build-tagged, opt-in).
  - Add a test fixture that checks an app container consuming the proxy through `ALL_PROXY`.
  - Keep live external trace checks opt-in (`CFWARP_TEST_LIVE=1`) so default CI remains deterministic.
